...
\begin{figure}[H]
	\begin{lstlisting}[language=Python]		
		...
		class BaldursGame(Game):
			...
			self.add_pc\_to\_team(Mage(0, 0, 0))
...
\begin{figure}[H]
	\begin{lstlisting}[language=Python]
		...
		class Portal(Object):
			def __ini\__(self, x, y, width, height, area, team_x, team_y):
				self.states = None
				self.sprite = None
			...
			def actor_in(self, actor):
				if actor.category == "pc":
					...
					Game.game.set_team(self.team\_x, self.team_y, 100)
				actor.stop_move()		
		модуль ruins
		import random
		from math import sqrt
	...
	\begin{lstlisting}[language=Python]
		...
		class Ruins(Area):
			def __init__(self):
				...
				self.add_object(self.grunt, 500, 185, 1)
				p = Portal(400, 400, 200, 200, 'Village', 480, 100)
				self.add\_object(p, p.pos_x, p.pos_y, 100)
				Game.game.start_script(self.ai, "ai", self.grunt)
				Game.game.start_script(self.walk_two, "footman", 50, 50)	
			def ai(self, actor):
				if actor.hp <= 0:
